...
export abstract class BaseService<T>{
    ...
    getAllDeleted() {
        return this.httpClient.delete<DataResponse<T[]>>(environment.getApiUrl(`/${this.path}/get-all-deleted`))
    }
    getAllNotDeleted() {
        return this.httpClient.delete<DataResponse<T[]>>(environment.getApiUrl(`/${this.path}/get-all-not-deleted`))
    }
    hardDeleteById(id: number) {
        return this.httpClient.delete<DataResponse<T[]>>(environment.getApiUrl(`/${this.path}/hard-delete-by-id/` + id))
    }
    restoreById(id: number) {
        return this.httpClient.delete<DataResponse<T[]>>(environment.getApiUrl(`/${this.path}/restore-by-id/` + id))
    }
}
